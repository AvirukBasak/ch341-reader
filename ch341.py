...
# flags for IO-Bits
# ---------------------------------------------------------------------------
_CH341_BIT_RTS = _SET_BIT(6)
_CH341_BIT_DTR = _SET_BIT(5)
...
_CH341_MULT_STAT = 0x04  # multiple status since last interrupt event
...
_CH341_REQ_WRITE_REG    = 0x9A
_CH341_REQ_READ_REG     = 0x95
_CH341_REQ_SERIAL_INIT  = 0xA1
_CH341_REQ_MODEM_CTRL   = 0xA4

_CH341_REG_BREAK     = 0x05
_CH341_REG_PRESCALER = 0x12
_CH341_REG_DIVISOR   = 0x13
_CH341_REG_LCR       = 0x18
_CH341_REG_LCR2      = 0x25
_CH341_REG_FLOW_CTL  = 0x27

_CH341_NBREAK_BITS = 0x01

_CH341_LCR_ENABLE_RX   = 0x80
_CH341_LCR_ENABLE_TX   = 0x40
_CH341_LCR_MARK_SPACE  = 0x20
_CH341_LCR_PAR_EVEN    = 0x10
_CH341_LCR_ENABLE_PAR  = 0x08
_CH341_LCR_STOP_BITS_2 = 0x04
_CH341_LCR_CS8         = 0x03
_CH341_LCR_CS7         = 0x02
_CH341_LCR_CS6         = 0x01
_CH341_LCR_CS5         = 0x00

_CH341_FLOW_CTL_NONE   = 0x00
_CH341_FLOW_CTL_RTSCTS = 0x01

_CH341_QUIRK_LIMITED_PRESCALER = _SET_BIT(0)
_CH341_QUIRK_SIMULATE_BREAK    = _SET_BIT(1)

# ---------------------------------------------------------------------------
# Clock / baud-rate constants
# ---------------------------------------------------------------------------
_CH341_CLKRATE = 48_000_000

def _CH341_CLK_DIV(ps: int, fact: int) -> int:
    return 1 << (12 - 3 * ps - fact)

def _CH341_MIN_RATE(ps: int) -> int:
    return _CH341_CLKRATE // (_CH341_CLK_DIV(ps, 1) * 512)
...
_CH341_MIN_BPS = _div_round_up(_CH341_CLKRATE, _CH341_CLK_DIV(0, 0) * 256)
_CH341_MAX_BPS = _CH341_CLKRATE // (_CH341_CLK_DIV(3, 0) * 2)

# ---------------------------------------------------------------------------
# Endpoint addresses (standard for CH340/CH341)
# ---------------------------------------------------------------------------
_CH341_BULK_IN_EP  = 0x82   # bulk IN
_CH341_BULK_OUT_EP = 0x02   # bulk OUT
...
class _CH341Private:
    def __init__(self):
        self.baud_rate: int  = CH341_DEFAULT_BAUDRATE
...
def _ch341_control_out(dev, request: int, value: int, index: int) -> int:
    r = usb_control_msg(
        dev,
...
def _ch341_control_in(dev, request: int, value: int, index: int,
...
def _ch341_get_divisor(priv: _CH341Private, speed: int) -> int:
    """
    ...
    #
    speed = max(_CH341_MIN_BPS, min(speed, _CH341_MAX_BPS))
    ...
    clk_div = _CH341_CLK_DIV(ps, fact)
    div = _CH341_CLKRATE // (clk_div * speed)

    # Some devices require a lower base clock if ps < 3.
    force_fact0 = False
    if ps < 3 and (priv.quirks & _CH341_QUIRK_LIMITED_PRESCALER):
        force_fact0 = True
    ...
    #
    if (16 * _CH341_CLKRATE // (clk_div * div) - 16 * speed >=
            16 * speed - 16 * _CH341_CLKRATE // (clk_div * (div + 1))):
        div += 1
...
def _ch341_set_baudrate_lcr(dev, priv: _CH341Private,
                           baud_rate: int, lcr: int) -> int:
    if not baud_rate:
        return -22  # -EINVAL

    val = _ch341_get_divisor(priv, baud_rate)
    if val < 0:
        return -22  # -EINVAL
    ...
    r = _ch341_control_out(
        dev,
        _CH341_REQ_WRITE_REG,
        _CH341_REG_DIVISOR << 8 | _CH341_REG_PRESCALER,
        val,
    )
    if r:
        return r
    ...
    r = _ch341_control_out(
        dev,
        _CH341_REQ_WRITE_REG,
        _CH341_REG_LCR2 << 8 | _CH341_REG_LCR,
        lcr,
    )
    return r


def _ch341_set_handshake(dev, control: int) -> int:
    return _ch341_control_out(dev, _CH341_REQ_MODEM_CTRL, (~control) & 0xFF, 0)


def ch341_get_status(dev, priv: _CH341Private) -> int:
    size = 2
    buffer = bytearray(size)
    r = _ch341_control_in(dev, _CH341_REQ_READ_REG, 0x0706, 0, buffer, size)
    if r:
        return r
    priv.msr = (~buffer[0]) & _CH341_BITS_MODEM_STAT
    return 0
...
def _ch341_configure(dev, priv: _CH341Private) -> int:
    size = 2
    buffer = bytearray(size)

    # expect two bytes 0x27 0x00
    r = _ch341_control_in(dev, _CH341_REQ_READ_VERSION, 0, 0, buffer, size)
    if r:
        return r
    ...
    r = _ch341_control_out(dev, _CH341_REQ_SERIAL_INIT, 0, 0)
    if r < 0:
        return r

    r = _ch341_set_baudrate_lcr(dev, priv, priv.baud_rate, priv.lcr)
    if r < 0:
        return r

    r = _ch341_set_handshake(dev, priv.mcr)
    if r < 0:
        return r
...
def _ch341_detect_quirks(dev, priv: _CH341Private) -> int:
    """
    ...
    if r == EPIPE:
        ...
        quirks = _CH341_QUIRK_LIMITED_PRESCALER | _CH341_QUIRK_SIMULATE_BREAK
        r = 0
    elif r:
...
def ch341_carrier_raised(handle: CH341Device) -> bool:
    """Return True if DCD (carrier detect) is asserted."""
    return bool(handle._priv.msr & _CH341_BIT_DCD)


def ch341_dtr_rts(dev, priv: _CH341Private, on: bool) -> None:
    """Assert or de-assert DTR and RTS lines."""
    # drop DTR and RTS
    if on:
        priv.mcr |= _CH341_BIT_RTS | _CH341_BIT_DTR
    else:
        priv.mcr &= ~(_CH341_BIT_RTS | _CH341_BIT_DTR)
    _ch341_set_handshake(dev, priv.mcr)
...
def ch341_update_status(handle: CH341Device, data: bytes) -> dict:
    """
    ...
    status = (~data[2]) & _CH341_BITS_MODEM_STAT

    delta = status ^ handle._priv.msr
    handle._priv.msr = status

    if data[1] & _CH341_MULT_STAT:
    ...
    changes = {}
    if delta & _CH341_BIT_CTS:
        changes["cts"] = bool(status & _CH341_BIT_CTS)
    if delta & _CH341_BIT_DSR:
        changes["dsr"] = bool(status & _CH341_BIT_DSR)
    if delta & _CH341_BIT_RI:
        changes["ri"]  = bool(status & _CH341_BIT_RI)
    if delta & _CH341_BIT_DCD:
        changes["dcd"] = bool(status & _CH341_BIT_DCD)
...
def ch341_read(handle: CH341Device, size: int = 64,
    ...
    if mps > 0 and size < mps:
        ...
        size = mps
    result = usb_bulk_read(handle._dev, _CH341_BULK_IN_EP, size, timeout)
...
def ch341_read_interrupt(handle: CH341Device,
    ...
    """
    result = usb_interrupt_read(handle._dev, _CH341_INT_IN_EP, 4, timeout)
    if isinstance(result, int):
        log.debug("ch341_read_interrupt: no data (errno %d)", result)
        return {}
    return ch341_update_status(handle, result)
...
def ch341_open(vendor_id: int = CH341_VENDOR_ID,
               ...
               baud_rate: int = CH341_DEFAULT_BAUDRATE) -> CH341Device:
    """
    ...
    #
    priv.lcr = _CH341_LCR_ENABLE_RX | _CH341_LCR_ENABLE_TX | _CH341_LCR_CS8

    r = _ch341_configure(udev, priv)
    if r < 0:
    ...
    r = _ch341_detect_quirks(udev, priv)
    if r < 0:
        usb_release_interface(udev, 0)
    ...
    handle._bulk_in_max_packet_size = mps
    return handle
...
def ch341_close(handle: CH341Device) -> None:
    ...
    """
    if handle._dev is None:
        return
    try:
        ch341_dtr_rts(handle._dev, handle._priv, False)
    except Exception:
        pass
    usb_release_interface(handle._dev, 0)
    handle._dev = None
...
def ch341_set_baud_rate(handle: CH341Device, baud_rate: int) -> int:
    ...
    handle._priv.baud_rate = baud_rate
    return _ch341_set_baudrate_lcr(handle._dev, handle._priv,
                                  baud_rate, handle._priv.lcr)
...
def ch341_set_line_control(handle: CH341Device, lcr: int) -> int:
    ...
    handle._priv.lcr = lcr
    return _ch341_set_baudrate_lcr(handle._dev, handle._priv,
                                  handle._priv.baud_rate, lcr)
...
def ch341_reset_resume(handle: CH341Device) -> int:
    ...
    """
    r = _ch341_configure(handle._dev, handle._priv)
    if r:
        return r
    r = ch341_get_status(handle._dev, handle._priv)
    if r < 0:
...
def _ch341_cleanup_handle(handle: CH341Device) -> None:
    ch341_close(handle)
